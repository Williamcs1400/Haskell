...
converterNotaParaMencao a | a >= 9 && a <= 10 = "SS"
                          | a >= 7 && a <= 8.9 = "MS"
                          | a >= 5 && a <= 6.9 = "MM"
                          | a >= 3 && a <= 4.9 = "MI"
                          | a >= 0.1 && a <= 2.9 = "II"
...
isDecrescente (x:y:xs) | x <= y = False
                       | otherwise = isDecrescente (y:xs)
...
histograma [] = []
histograma (x:xs) = (x, countLista x xs) : histograma(removeLista x xs)

countLista :: String -> [String] -> Int
countLista _ [] = 1
countLista elemento (x:xs) | elemento == x = 1 + countLista elemento xs
                           | otherwise = countLista elemento xs

removeLista :: String -> [String] -> [String]
removeLista _ [] = []
removeLista x (y:ys) | x /= y = y : removeLista x ys
                     | otherwise  = removeLista x ys
...
aprovadosOrdemDeMedia ((nome, nota1, nota2):ns) | (nota1 + nota2)/2 >= 5 = mySort((nome, (nota1 + nota2)/2) : aprovadosOrdemDeMedia ns)
                                                | otherwise = aprovadosOrdemDeMedia ns
...
somaMatricial [] [] = []
somaMatricial (x:xs) (y:ys) = somaVetor x y : somaMatricial xs ys

somaVetor :: Num u => [u] -> [u] -> [u]
somaVetor [] [] = []
somaVetor (a:as) (b:bs) = a + b : somaVetor as bs

matrizTransposta :: Num u => [[u]] -> [[u]]
matrizTransposta [] = [[]]
matrizTransposta [[]] = [[]]
matrizTransposta [x:y:xs]  = [x] : matrizTransposta [y:xs]
